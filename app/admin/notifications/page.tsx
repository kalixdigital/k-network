...
export default function AdminNotificationsPage() {
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      // Get all notifications for admin using direct query (not just unread)
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-0">
        ...
        <div className="flex items-center gap-2">
          {unreadCount > 0 && (
            <button
              ...
              className={`rounded-lg border ${levelData.borderColor} px-4 py-2 text-sm font-medium ${levelColor} hover:bg-slate-800 transition`}
            >
              Mark all read
            </button>
          )}
      ...
      {notifications.length === 0 ? (
        <div className={`rounded-2xl border ${levelData.borderColor} bg-slate-900/50 p-12 text-center shadow-xl backdrop-blur`}>
            ...
            return (
              <div
                ...
                className={`rounded-xl border p-4 transition ${
                  isUnread 
                    ? `${levelData.borderColor} ${levelData.lightBg}` 
